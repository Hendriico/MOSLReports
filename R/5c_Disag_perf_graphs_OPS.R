graphs_OPS <- function(dir=choose.dir(),output_dir=dir){
  ...
  for (x in TP_list) {
    ...
    if(unique(ops_data_graph$OPS) %in% c("OPS B5a", "OPS C1a")){
      ...
      ggsave(file = paste(output_dir, "\\graphs\\OPS\\", x, "_KPI.png", sep = ""),plot.taskcompletion, width = 7, height = 5)
      ...
      ggsave(file = paste(output_dir, "\\graphs\\OPS\\", x, "_API.png", sep = ""),plot.outstanding, width = 7, height = 5)
